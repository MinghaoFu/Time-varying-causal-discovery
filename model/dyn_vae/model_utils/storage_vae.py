...
class RolloutStorageVAE(object):
    def __init__(self, num_processes, max_trajectory_len, zero_pad, max_num_rollouts,
        ...
        if self.max_buffer_size > 0:
            ...
            self.curr_state = torch.zeros((self.max_traj_len, self.max_buffer_size, state_dim))
            self.next_state = torch.zeros((self.max_traj_len, self.max_buffer_size, state_dim))
            self.prev_actions = torch.zeros((self.max_traj_len, self.max_buffer_size, action_dim))
            self.curr_actions = torch.zeros((self.max_traj_len, self.max_buffer_size, action_dim))
            self.next_actions = torch.zeros((self.max_traj_len, self.max_buffer_size, action_dim))
            self.curr_rew = torch.zeros((self.max_traj_len, self.max_buffer_size, 1))
            self.next_rew = torch.zeros((self.max_traj_len, self.max_buffer_size, 1))
        ...
        self.running_curr_state = torch.zeros((self.max_traj_len, num_processes, state_dim)).to(device)
        self.running_next_state = torch.zeros((self.max_traj_len, num_processes, state_dim)).to(device)
        self.running_curr_rewards = torch.zeros((self.max_traj_len, num_processes, 1)).to(device)
        self.running_next_rewards = torch.zeros((self.max_traj_len, num_processes, 1)).to(device)
        self.running_prev_actions = torch.zeros((self.max_traj_len, num_processes, action_dim)).to(device)
        self.running_curr_actions = torch.zeros((self.max_traj_len, num_processes, action_dim)).to(device)
        self.running_next_actions = torch.zeros((self.max_traj_len, num_processes, action_dim)).to(device)
    ...
    def get_running_batch(self):
        return self.running_prev_state, self.running_curr_state, self.running_next_state, \
               self.running_prev_actions, self.running_curr_actions, self.running_next_actions, \
               self.running_curr_rewards, self.running_next_rewards, self.curr_timestep
